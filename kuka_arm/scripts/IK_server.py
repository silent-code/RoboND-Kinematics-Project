...
def handle_calculate_IK(req):
    ...
    # roll, pitch, yaw = end-effector orientation
        ...
        Rot_ee = Rot_ee.subs({'r': roll, 'p': ppitch, 'y': yaw})
        EE = Matrix ([[px],[py],[pz]])
        WC = EE - .303 * Rot_ee[:, 2]
        ...
        side_a  = 1.5
        sibe_b = sqrt(pow((sqrt(WC[0] * WC[0] + WC[1] * WC[1]) - .35),2) + pow((WC[2] - .75), 2))
        side_c = 1.25
        angle_a = acos((side_b * side_b + side_c * side_c - side_a * side_a) / (2* side_b * side_c))
        angle_b = acos((side_a * side_a + side_c * side_c - side_b * side_b) / (2 * side_a * side_c))
        angle_c = acos((side_a * side_a + siode_b * side_b - side_c * side_c) / (2 * side_a * side_b))
